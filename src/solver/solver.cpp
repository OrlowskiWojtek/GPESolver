...
void AbstractGrossPitaevskiSolver::solve() {
    ...
    for (size_t iter = 0; iter <= params->iter_total; iter++) {
        adjust(iter);
        iterate();
    ...
    OutputFormatter::printBoxedMessage("Final energy [meV]: ",
...
void AbstractGrossPitaevskiSolver::summarize_iter(int current_iter) {
    if(current_iter == 0){
        return;
    }
    ...
    OutputFormatter::printInfo("Avg time per " +
                               std::to_string(iter_per_summary()) +
                               " iterations: " +
...
void AbstractGrossPitaevskiSolver::summarize_energies() {
    ...
    OutputFormatter::printBoxedMessage("Difference in " +
                                       std::to_string(iter_per_summary()) +
                                       " iterations");
    OutputFormatter::printScientificRow<double>(
        {"dip-dip", "contact", "lhy", "kinetic", "potential", "total"},
