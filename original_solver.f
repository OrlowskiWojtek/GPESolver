...
c     call lifi3(fi3d,cpsi,n,ny,nz,dx,dz,xnorma(1),xncz(1),rdy)
     ...
     >-2*cpsi(ix,iy,iz))
      c2=
     >+cpsi(ix,iy,iz)*(vvv+0*cdd*fi3d(ix,iy,iz)/2)
      c3=
     >+cpsi(ix,iy,iz)*(0*vvv+cdd*fi3d(ix,iy,iz)/2)
      ...
      do iz=-nz+1,nz-1
      e3=e3-cdd/3
     >*cdabs(cpsi(ix,iy,iz))**4*dx**2/2*xncz(1)**2/xnorma(1)**2*dz
      e4=e4+
     >(ggp11-0*cdd/3)
     >*cdabs(cpsi(ix,iy,iz))**4*dx**2/2*xncz(1)**2/xnorma(1)**2*dz
      e5=e5
     >+gamma*cdabs(cpsi(ix,iy,iz))**5*w**2.5*.4*dx**2*dz
      enddo
      enddo
      enddo
      cpsin=cpsii
...
c     st=system("date")
      ...
      fi(ix,iy,iz)=fi(ix,iy,iz)+
     >rdijk*psi(ixp,iyp,izp)*xfct
      enddo
      enddo
      enddo
      enddo
...
c     st=system("date")
     ...
     >+psi(ix,iy,iz)*dx**2*dz**2*xncz/xnorma
     >)
     >/(4*dz**2+2*dx**2)
      enddo
      enddo
      ...
      do iz=-Nz+1,Nz-1
      fi3d(ix,iy,iz)=
     >-(fi(ix,iy,iz+1)+fi(ix,iy,iz-1)-2*fi(ix,iy,iz))/dz**2
      enddo
      enddo
      enddo
c     st=system("date")
